...
const validCases = [
  ...
  {
    name: "Axal product EvidencePanel contract",
    brief: {
      id: "axal-product-evidence-panel",
      consumerMode: "axal/workspace",
      register: "product",
      surface: "Product evidence review",
      userJob: "Review source evidence with the canonical S7 product contract.",
      primaryDecision: "Can the named reviewer continue?",
      dataSensitivity: ["tenant-data"],
      trustBoundaries: ["Tenant-scoped workspace data only"],
      evidenceRequirements: ["Source evidence", "Human review state"],
      selectedPatterns: [{ catalog: "product", name: "EvidencePanel", states: ["under-review"] }],
      claims: [{ claim: "Evidence is under review", evidence: "A source and named reviewer are visible" }],
      nonGoals: ["No filing submission", "No autonomous approval"],
      verificationGates: ["keyboard-focus"],
    },
  },
  {
    name: "Canonical ProductFamilyRouter alias",
    brief: {
      id: "canonical-product-family-router",
      consumerMode: "complyeaze/core",
      register: "brand",
      surface: "Canonical product route map",
      userJob: "Compare the product routes and their boundaries.",
      primaryDecision: "Which operating route fits the work?",
      dataSensitivity: ["public-copy"],
      trustBoundaries: ["Public copy only", "No auth, tenant, document, or workspace behavior"],
      evidenceRequirements: ["Package contract"],
      selectedPatterns: [{ catalog: "product", name: "ProductFamilyRouter", states: ["default"] }],
      claims: [{ claim: "The alias resolves to ProductRouteMap", evidence: "The canonical product registry owns the contract" }],
      nonGoals: ["No auth flow", "No tenant data", "No document handling", "No workspace behavior"],
      verificationGates: ["token-only-styling"],
    },
  },
  {
